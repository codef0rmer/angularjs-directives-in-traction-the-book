...
This chapter covers
* The difference between Compile and Link, and their purpose
* Transcluding an external markup to customize template used by directive
...
<head>
  ...
  <script type="text/ng-template" id="thumbviewer.html">			#A
    <div class='thumbnail-container bg-success img-rounded'>
      <div class='thumbnail-wrapper'>
        <!-- Page Thumbs go here -->					#B
      </div>
    </div>
  </script>
  <script type="text/ng-template" id="thumb.html">			#A
    <div class="thumbnail pull-left"></div>
  </script>
</head>

<body style="padding: 10px;" ng-controller="ThumbCtrl">
  <thumbviewer data-size="150" data-gap="0"></thumbviewer>		#C
</body>
</html>
```
...
App.controller('ThumbCtrl', function($scope) {
  $scope.thumbnails = [
    { title: 'Page 1',  image: '1.png', active: true },			#A
...
App.directive('thumbviewer', function() {
  return {
    restrict: 'EA',
    replace: true,
    templateUrl: 'thumbviewer.html',					#B
    scope: {
      size: '=?',								#C
      gap : '=?'								#C
    }
...
So, all the complex things that do not require scope can be accomplished in the compile phase for the performance gain and other benefits are:
...
* Unlike compile function, the link function receives 3 parameters, namely, scope, element, and attributes. Feel free to rename them as you wish but the order should be intact.
* If an attribute consists of an AngularJS model that is data-index='index', it can be extracted after the evaluation using scope.$eval(attrs['index']) or $parse(attrs['index'])(scope) service. If it contains an interpolated string such as data-index='{{index}}', the observer function (attrs.$observe) should be used instead and will be covered in Chapter 7.
...
<thumbviewer data-size="150" data-gap="0">
  <thumb ng-repeat="thumbnail in thumbnails" active="thumbnail.active">	#A
    <div><img ng-src="../img/{{thumbnail.image}}"/></div>		#B
    <div class="caption text-center" ng-bind="thumbnail.title"></div>	#B
  </thumb>
</thumbviewer>
```
...
Now you will be excited to see thumbViewer quite working as per the following figure.
...
So it's upto you what syntax to use next time. Given that both the controller as well as link functions are quite similar, what's the functional difference between them? They are:
...
App.directive('foo', function() {
  return {
    restrict: 'EA',
    require: 'sameElement', // or '^parentElement'			#A
...
App.directive('thumb', function() {
  return {
    ...
    templateUrl: 'thumb.html',
    transclude: true,
    require: '^thumbviewer',						#A
    scope: {
      active  : '=?'
    },
    compile: function(tElement, tAttrs) {
      return function postLink(scope, element, attrs, thumbviewerCtrl) {	#B
        thumbviewerCtrl.addThumb(scope);					#C

        scope.isFirstThumb = function(thumb) {
          return thumbviewerCtrl.thumbs.indexOf(thumb);
        };    

        scope.thumbnailStyle = function(thumb) {				#D
          return {
        ...
        scope.select = function(selectedThumb) {				#E
...
Note that the link method runs for every thumb item with an isolate scope to maintain active state for each thumb separately without overriding. Now that we have a  total number of thumbs rendered, we can use this information in wrapperStyle() method to dynamically calculate the width for the same, replacing the hardcoded values used temporarily. So, update the wrapperStyle() method as follows:
...
* The methods/properties defined on this object in the directive controller can not be accessed  by the same directive. Use scope object instead. That's why we binded thumbSize and thumbGap properties both on this and scope objects in ThumbViewerCtrl, so that it will be read by thumb as well as thumbviewer directives.

Undoubtedly, there are no navigation arrows to navigate to thumbs which are out of the viewport and not visible which we'll fix in the following section. All that means is that require  option allows us to inject the directive controller of any directive, off-course, including built-in directives. One of them is ngModel directive, let us explore.
...
App.directive('isUnique', function() {
  return {
    ...
    require: 'ngModel',
    link: function(scope, element, attrs, ngModelCtrl) {
      ngModelCtrl.$setValidity('uniqueEmail', true);					#A

      ngModelCtrl.$parsers.push(function(viewValue) {
        ngModelCtrl.$setValidity('uniqueEmail', !(viewValue === 'foo@bar.com'));	#B
...
```
Nothing has changed much, we are simply replacing the occurrences of scope.default with ngModelCtrl.$modelValue and finally updating the viewValue with the help of setViewValue call. Then update inc and dec method slightly so:
...
This is very simplest form of $watch expression that re-renders the spinner UI whenever list model changes, we are going to cover in detail in next chapter. Finally we'll update ScopeCtrl to define arrWeekday used before in the DOM and their callbacks so:
...
Be happy to see all tests in green..! In the next section, we'll use the spinner directive to extend thumbviewer directive.
...
$scope.$watch(function() { return $scope.thumbs.length; }, function(newVal, oldVal) {									#A
  if (newVal || newVal !== oldVal) {					#B
    $scope.thumbList = [];
    for (var i = 0; i < newVal; i++) {
      $scope.thumbList.push(i + 1);					#B
    }
  }
});
...
scope.marginLeft = 0;							#A

scope.wrapperStyle = function() {
  var totalSize = scope.thumbSize * scope.thumbs.length;
  var totalGap  = scope.thumbGap * (scope.thumbs.length - 1);
  var scrollLeft = scope.thumbTracker*(scope.thumbSize + scope.thumbGap);#B
  var leftPos = (scrollLeft + scope.thumbSize + scope.thumbGap * 2 + scope.marginLeft);								 	#C
  var isInViewport = leftPos !== scope.thumbGap && leftPos < window.innerWidth;								 		#D

  if (!isInViewport && scrollLeft >= 0) {					 #E
  ...
  return {
    width   : totalSize + totalGap + 'px',
    display : 'inline-block',
    marginLeft: scope.marginLeft	 + 'px'					 #F
  };
};
```
...
Why ptor.sleep and what it does? If you recall, we had used 500ms in CSS transition to last the animation for 0.5 second. So when we navigate to a thumb that requires transition to freeze, we wait for 500ms and then check it's position to validate its visibility. The ptor.sleep() is useful to make the driver sleep for the given amount of time in order to schedule commands later.
...
As we have tested thumb navigation using the Spinner directive, we could also navigate to specific thumb with manual clicks. Let us test that too. Replace the comment added previously with the following snippet as:
...
## Summary
That was handful..! In this chapter,
